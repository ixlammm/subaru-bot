...
module.exports = class Track {
    ...
    if (parseInt(this.id)<0){
      ...
      this.filesize = {
        ...
        flac: parseInt(body.FILESIZE_FLAC),
				mp4_ra1: parseInt(body.FILESIZE_MP4_RA1),
				mp4_ra2: parseInt(body.FILESIZE_MP4_RA2),
				mp4_ra3: parseInt(body.FILESIZE_MP4_RA3)
      }
	      ...
	      body.ARTISTS.forEach(artist=>{
	        if (artist.__TYPE__ == "artist") this.artists.push({
	          ...
	          picture: artist.ART_PICTURE
				...
				this.artistsArray = []
        if (body.ART_NAME){
          this.artistsArray.push(body.ART_NAME)
        }
				if (body.SNG_CONTRIBUTORS.main_artist){
	        this.artistsArray = this.artistsArray.concat(body.SNG_CONTRIBUTORS.main_artist)
	      }else if (body.SNG_CONTRIBUTORS.mainartist){
	        this.artistsArray = this.artistsArray.concat(body.SNG_CONTRIBUTORS.mainartist)
	      }
				if (body.SNG_CONTRIBUTORS.featuredartist) {
					this.artistsArray = this.artistsArray.concat(body.SNG_CONTRIBUTORS.featuredartist)
				}
        if (body.SNG_CONTRIBUTORS.featuring) {
					this.artistsArray = this.artistsArray.concat(body.SNG_CONTRIBUTORS.featuring)
				}
	      if (body.SNG_CONTRIBUTORS.associatedperformer) {
	        this.artistsArray = this.artistsArray.concat(body.SNG_CONTRIBUTORS.associatedperformer)
	      }
			}
      this.gain = body.GAIN
      this.discNumber = body.DISK_NUMBER
      this.trackNumber = body.TRACK_NUMBER
			if (body.EXPLICIT_LYRICS){
				this.explicit = body.EXPLICIT_LYRICS != "0"
			}
      this.ISRC = body.ISRC
      this.contributor = body.SNG_CONTRIBUTORS
      this.lyricsId = body.LYRICS_ID
			this.copyright = body.COPYRIGHT
	    this.album.recordType = body.TYPE
	    this.contributor = body.SNG_CONTRIBUTORS
			if (body.LYRICS){
	      this.unsyncLyrics = {
	  			description: "",
  ...
  getDownloadUrl(format){
    if (!this.MD5) throw new Error("Authentication needed to get download URL")
